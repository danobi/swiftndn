...
open class FaceID: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.FaceID
    }
}

open class LocalControlFeature: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.LocalControlFeature
    }
}

open class Origin: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.Origin
    }
}

open class Cost: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.Cost
    }
}

open class Flags: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.Flags
    }
}

open class ExpirationPeriod: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.ExpirationPeriod
    }
}

open class ControlParameters: Tlv.Block {
    
    open var name: Name?
    open var faceID: FaceID?
    open var origin: Origin?
    open var cost: Cost?
    open var flags: Flags?
    open var expirePeriod: ExpirationPeriod?
    ...
    open class func wireDecode(_ bytes: [UInt8]) -> ControlParameters? {
...
open class StatusCode: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.StatusCode
    }
}

open class StatusText: StringTlv {
    override var tlvType: UInt64 {
        return NFDType.StatusText
    }
}

open class ControlResponse: Tlv.Block {
    
    open var statusCode = StatusCode()
    open var statusText = StatusText()
    ...
    open class func wireDecode(_ bytes: [UInt8]) -> ControlResponse? {
        let (block, _) = Tlv.Block.wireDecodeWithBytes(bytes)
        if let blk = block {
...
open class ControlCommand: SignedInterest {
    
    open var module: Name.Component!
    open var verb: Name.Component!
    open var parameters: ControlParameters!
    ...
    open override var prefix: Name {
        return self.name.getPrefix(self.name.size - 7)
    }
}

open class Uri: StringTlv {
    override var tlvType: UInt64 {
        return NFDType.Uri
    }
...
open class FaceScope: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
...
open class FacePersistency: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
...
open class LinkType: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
...
open class NInInterests: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.NInInterests
    }
}

open class NInDatas: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.NInDatas
    }
}

open class NOutInterests: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.NOutInterests
    }
}

open class NOutDatas: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.NOutDatas
    }
}

open class NInBytes: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.NInBytes
    }
}

open class NOutBytes: NonNegativeIntegerTlv {
    override var tlvType: UInt64 {
        return NFDType.NOutBytes
    }
}

open class FaceStatus: Tlv.Block {
    
    open var faceID = FaceID()
    open var uri = Uri()
    open var localUri = LocalUri()
    open var expirePeriod: ExpirationPeriod?
    open var faceScope = FaceScope()
    open var facePersistency = FacePersistency()
    open var linkType = LinkType()
    open var nInInterests = NInInterests()
    open var nInDatas = NInDatas()
    open var nOutInterests = NOutInterests()
    open var nOutDatas = NOutDatas()
    open var nInBytes = NInBytes()
    open var nOutBytes = NOutBytes()
    ...
    open class func parseFaceStatusDataset(_ bytes: [UInt8]) -> [FaceStatus] {
        var ret = [FaceStatus]()
...
open class RibEntry: Tlv.Block {
    ...
    open class func parseRIBDataset(_ bytes: [UInt8]) -> [RibEntry] {
        var ret = [RibEntry]()
        if let blocks = Tlv.Block.wireDecodeBlockArray(bytes) {
            for blk in blocks {
...
open class NextHopRecord: Tlv.Block {
...
open class FibEntry: Tlv.Block {
    open var name = Name()
    open var nexthops = [NextHopRecord]()
    ...
    open class func parseFIBDataset(_ bytes: [UInt8]) -> [FibEntry] {
        var ret = [FibEntry]()
        if let blocks = Tlv.Block.wireDecodeBlockArray(bytes) {
            for blk in blocks {
