...
open class Buffer: CustomStringConvertible {
    ...
    open func writeByte(_ byte: UInt8) -> Buffer {
        buffer.append(byte)
        return self
    }
    
    open func writeByteArray(_ bytes: [UInt8]) -> Buffer {
        for b in bytes {
            writeByte(b)
        }
        return self
    ...
    open class func getVarNumberEncodedLength(_ number: UInt64) -> UInt64 {
        switch number {
    ...
    open func writeVarNumber(_ number: UInt64) -> Buffer {
        switch number {
    ...
    open func peekVarNumber() -> (number: UInt64, length: Int)? {
    ...
    open func readVarNumber() -> (number: UInt64, length: Int)? {
    ...
    open class func getNonNegativeIntegerEncodedLength(_ number: UInt64) -> UInt64 {
        switch number {
    ...
    open class func byteArrayFromNonNegativeInteger(_ number: UInt64) -> [UInt8] {
        switch number {
    ...
    open class func nonNegativeIntegerFromByteArray(_ bytes: [UInt8]) -> UInt64 {
        var number: UInt64 = 0
        for b in bytes {
    ...
    open class func byteArrayFromString(_ string: String) -> [UInt8]? {
        if let data = (string as NSString).data(using: String.Encoding.ascii.rawValue) {
            var arr = [UInt8](repeating: 0, count: data.count)
            (data as NSData).getBytes(&arr, length: data.count)
            return arr
        } else {
            return nil
        }
    ...
    open class func stringFromByteArray(_ bytes: [UInt8]) -> String? {
    ...
    open func readByteArray(_ length: Int) -> (array: [UInt8], length: Int)? {
