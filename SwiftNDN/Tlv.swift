...
open class Tlv {
    ...
    public struct NDNType {
        ...
        public static let AnyType: UInt64 = 19
    ...
    open class Block {
        ...
        open var type: UInt64 = 0
        open var value = [UInt8]()
        
        open var length: UInt64 {
            return UInt64(value.count)
        }
        
        open var totalLength: UInt64 {
            let l = self.length
        ...
        open func wireEncode() -> [UInt8] {
            let buf = Buffer(capacity: Int(self.totalLength))
            self.wireEncode(buf)
            return buf.buffer
        }
        
        func wireEncode(_ buf: Buffer) {
            buf.writeVarNumber(self.type)
                .writeVarNumber(self.length)
                .writeByteArray(self.wireEncodeValue())
        }
        
        open func wireEncodeValue() -> [UInt8] {
            return self.value
        }
        
        open class func wireDecodeWithBytes(_ bytes: [UInt8]) -> (block: Block?, lengthRead: Int) {
            let buf = Buffer(buffer: bytes)
            return wireDecodeWithBuffer(buf)
        }
        
        open class func wireDecodeWithBuffer(_ buf: Buffer) -> (block: Block?, lengthRead: Int) {
        ...
        open class func wireDecodeBlockArray(_ bytes: [UInt8]) -> [Block]? {
            var bytesRead: Int = 0
            let buf = Buffer(buffer: bytes)
            var ret = [Block]()
...
open class NonNegativeIntegerTlv: Tlv.Block {
    ...
    open var integerValue: UInt64 = 0
...
open class StringTlv: Tlv.Block {
