...
open class Name: Tlv.Block {
    
    open class Component: Tlv.Block {
        ...
        public init?(url: String) {
            ...
            while index != url.utf8.endIndex {
                ...
                if codeByte == 0x25 {
                    index = url.utf8.index(after: index)
...
//                index = <#T##Collection corresponding to `index`##Collection#>.index(after: index)
        ...
        open func compare(_ target: Component) -> Int {
    ...
    public init?(url: String) {
        ...
        if let match = ndnNameRegex?.firstMatch(in: url, options: [],
            range: NSRange(location: 0, length: url.characters.count))
        {
            if match.numberOfRanges != 2 {
                return nil
            }
            let prefix = (url as NSString).substring(with: match.rangeAt(1))
            let stringComps = prefix.components(separatedBy: "/")
    ...
    open func appendComponent(_ component: Component) -> Name {
        self.components.append(component)
        return self
    }
    
    open func appendComponent(_ bytes: [UInt8]) -> Name {
        self.components.append(Component(bytes: bytes))
        return self
    }
    
    open func appendComponent(_ url: String) -> Name? {
    ...
    open func appendNumber(_ number: UInt64) -> Name {
        let arr = Buffer.byteArrayFromNonNegativeInteger(number)
        return self.appendComponent(arr)
    }
    
    open func getComponentByIndex(_ index: Int) -> Component? {
        var i = index
        if i < 0 {
    ...
    open func getPrefix(_ length: Int) -> Name {
        if length >= self.size {
    ...
    // Return -1 if self < target; +1 if self > target; 0 if self == target
    open func compare(_ target: Name) -> Int {
    ...
    open func isProperPrefixOf(_ name: Name) -> Bool {
        return self.isPrefixOf(name) && self.size < name.size
    }
    
    open func isPrefixOf(_ name: Name) -> Bool {
    ...
    open class func wireDecode(_ bytes: [UInt8]) -> Name? {
