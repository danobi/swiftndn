...
//        println(url4?.pathComponents?.count)
    ...
    func testInterest() {
        ...
        let d40 = SwiftNDN.Data()
        d40.name = Name(url: "/a/b/c")!
        let d41 = SwiftNDN.Data()
        d41.name = Name(url: "/a/b/c/%00%05")!
        let d42 = SwiftNDN.Data()
        d42.name = Name(url: "/a/b/c/%00%01")!
        let d43 = SwiftNDN.Data()
    ...
    func testData() {
        ...
        let keychainSignExpectation = expectation(description: "sign data")
        let keychainVerifyExpectation = expectation(description: "verify data")
        
        let keychain = KeyChain()
        XCTAssert(keychain != nil)
        
        keychain?.sign(d0, onFinish: { (signedData: SwiftNDN.Data) in
            keychainSignExpectation.fulfill()
            
            let d0Encode = signedData.wireEncode()
            print(d0Encode)
            
            let d1 = SwiftNDN.Data.wireDecode(d0Encode)
            XCTAssert(d1 != nil)
    ...
    func testTransport() {
        ...
        client.openExpectation = expectation(description: "open transport")
        client.receiveName1Expectation = expectation(description: "receive name1")
        client.receiveName2Expectation = expectation(description: "receive name2")
        client.receiveName3Expectation = expectation(description: "receive name3")
        client.receiveName4Expectation = expectation(description: "receive name4")
        client.receiveName5Expectation = expectation(description: "receive name5")
        client.closeExpectation = expectation(description: "close transport")
        client.run()
    ...
    func testTimer() {
        if let timer = SwiftNDN.Timer() {
            let timerExpectation = expectation(description: "fire timer")
    ...
    class FaceTestClient: FaceDelegate {
        ...
        func onI0Data(_ i0: Interest, d0: SwiftNDN.Data) {
            receiveI0DataExpectation?.fulfill()
            //XCTAssertEqual(d0.name.toUri(), "/a/b/c/%00%02")
            XCTAssertEqual(d0.name.toUri(), i0.name.toUri())
        ...
        func onI0Timeout(_ i0: Interest) {
            XCTFail("i0 timeout")
        }
        
        func onI01Data(_ i01: Interest, d01: SwiftNDN.Data) {
            receiveI01DataExpectation?.fulfill()
            //XCTAssertEqual(d01.name.toUri(), "/a/b/c/%00%02")
            XCTAssertEqual(d01.name.toUri(), i01.name.toUri())
            XCTAssert(d01.getContent() == [0, 1, 2, 3, 4, 5, 6, 7])
        }
        
        func onI01Timeout(_ i01: Interest) {
            XCTFail("i01 timeout")
        }
        
        func onI1Data(_ i1: Interest, d1: SwiftNDN.Data) {
            XCTFail("i1 data received")
        }
        
        func onI1Timeout(_ i1: Interest) {
    ...
    class RibRegisterTestClient: FaceDelegate {
        ...
        func onInterest(_ interest: Interest) {
        ...
        func onRegFailure(_ msg: String) {
    ...
    func testFace() {
        ...
        client.receiveI0DataExpectation = expectation(description: "receive i0 data")
        client.receiveI01DataExpectation = expectation(description: "receive i01 data")
        client.closeExpectation = expectation(description: "close client")
        client.run()
    ...
    func testFace2() {
        ...
        client2.registerSuccessExpectation = expectation(description: "register prefix")
        client2.receiveInterestExpectation = expectation(description: "receive interest")
        client2.closeExpectation = expectation(description: "close client")
        client2.run()
